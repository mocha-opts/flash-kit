...
**数据库层不使用物理外键约束（FOREIGN KEY / REFERENCES）。** 关联关系通过 Drizzle `relations()` 定义在应用层，引用列必须手动添加索引。

原因：

- 物理外键在每次写入时触发引用检查，高并发下显著影响性能
- `ON DELETE CASCADE` 静默级联删除，无应用层日志，不可控
- 外键导致迁移必须按依赖顺序执行，快速迭代时极其痛苦
- 交叉外键锁检查增加死锁概率（席位计费场景：成员变更 + 订阅更新并发）
- 阻碍未来迁移到分布式数据库（PlanetScale 直接不支持外键）

做法：

- Schema 中引用列只写 `uuid('user_id').notNull()`，不写 `.references()`
- 用 `relations()` 定义关联，Drizzle query API 依然支持 `with` 嵌套查询
- 每个引用列必须手动添加 `index()`
- 删除操作在应用层事务中按依赖顺序显式执行，附带审计日志
- 定期运行 `integrity-check` 脚本扫描孤儿数据
...
- organizations.ownerId: 逻辑引用 users.id，不加 FK 约束
- 所有 userId / orgId 列: notNull + index，不加 references
```

Schema 示例（正确写法）：

```typescript
// ✅ 正确：notNull + index，无 .references()
...
await db.transaction(async (tx) => {
  ...
  await tx
    .insert(auditLogs)
    .values({ action: "organization.deleted", targetId: orgId });
});
```
...
所有 UI 组件从 `@flash-kit/ui` 导入：

```typescript
import { Button } from "@flash-kit/ui/components/button";
import { cn } from "@flash-kit/ui/lib/utils";
```
...
Biome 自动排序。顺序：React/Next → 第三方库 → @flash-kit/\* 包 → 相对路径 → 类型导入。
...
安装/卸载通过 CLI：`npx flash-kit plugin add feedback`
...
每个 action 第一行调用 `requireOrgRole()`。页面通过 `useOrg()` 获取当前组织上下文。数据表添加 `orgId` 列（notNull + index，无物理外键）实现租户隔离。
...
- 不要在 Client Component 中 import `@flash-kit/database`
